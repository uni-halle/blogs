...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="entry-header">
		...
		<h3 class="entry-format"><?php _e( 'Gallery', 'catch-box' ); ?></h3>
	...
	<?php if ( is_search() ) : // Only display Excerpts for search pages ?>
		...
		<div class="entry-content">
			...
			<?php else : ?>
				...
				<p><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'catch-box' ),
						'href="' . esc_url( get_permalink() ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'catch-box' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark"',
