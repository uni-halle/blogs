<?php
	...
	// @llz - Alle Funktionen hier sind extra für das LLZ-Theme und waren nicht (!) im Parent-Theme

	// Auszugslänge auf 100 verringern
	function llz_excerpt_length( $length ) {
		return 100;
	}
	add_filter( 'excerpt_length', 'llz_excerpt_length', 999 );
	
	// Sprache auf deutsch ändern
	function change_locale(){ return 'de'; }
	add_filter( 'locale', 'change_locale' );

	// Query bearbeiten, damit er nach "Likes" sortiert
	...
	// Bei jedem Post legen wir eine 0 für die Likes an, damit Wordpress diese auch beachtet 
	function add_meta_to_new_post($post_id) {
		add_post_meta( $post_id, '_post_like_count', 0 );
	}
	add_action( 'publish_post', 'add_meta_to_new_post' );
	add_action( 'publish_page', 'add_meta_to_new_post' );
	
	// Nach einem Kommentar verweisen wir wieder auf die Seite, auf der er verfasst wurde anstatt auf single.php 
	function redirect_after_comment($location) {
		return $_SERVER["HTTP_REFERER"];
	}
	add_filter('comment_post_redirect', 'redirect_after_comment');
	
	// Benutzerdefiniertes Javascript einfügen
	function llz_add_custom_js() {
		wp_enqueue_script('llz_custom', get_stylesheet_directory_uri().'/assets/js/llz_custom.js', array('jquery'), '1.0', 1);
	}
	add_action('wp_enqueue_scripts', 'llz_add_custom_js');
	
	// get_the_content liefert standardmäßig nur Fließtext ohne die Formatierung. Daher diese Funktion
	function get_the_content_with_formatting ($more_link_text = '[mehr...]', $stripteaser = 0, $more_file = '') {
...
function like_scripts() {
	wp_enqueue_script( 'jm_like_post', get_stylesheet_directory_uri().'/assets/js/post-like.js', array('jquery'), '1.0', 1 );
...
function enqueue_icons () {
	wp_register_style( 'icon-style', 'https://netdna.bootstrapcdn.com/font-awesome/4.0.0/css/font-awesome.css' );
...
function st_like_shortcode() {
	return jm_most_popular();
}
add_shortcode('stliker', 'st_like_shortcode');
