 ...
 * Define Constants
 */
define('MAJA_SHORTNAME', 'maja'); // used to prefix the individual setting field id see maja_options_page_fields()
define('MAJA_PAGE_BASENAME', 'maja-settings'); // the settings page slug

/**
 * Specify Hooks/Filters
 */
add_action( 'admin_menu', 'maja_add_menu' );
add_action( 'admin_init', 'maja_register_settings' );

/**
 * Include the required files
 */
// page settings sections & fields as well as the contextual help text.
require_once('maja-theme-options.php');
 ...
 * @return array
 */
function maja_get_settings() {
	
	$output = array();
	
	// put together the output array 
	$output['maja_option_name'] 		= 'maja_options'; // the option name as used in the get_option() call.
	$output['maja_page_title'] 		= __( 'Theme Settings Page','maja'); // the settings page title
	$output['maja_page_sections'] 	= maja_options_page_sections(); // the setting section
	$output['maja_page_fields'] 		= maja_options_page_fields(); // the setting fields
 ...
 */
function maja_create_settings_field( $args = array() ) {
	...
	$defaults = array(
		...
		'section' => 'main_section', 					// the section this setting belongs to — must match the array key of a section in maja_options_page_sections()
	...
	// additional arguments for use in form field output in the function maja_form_field_fn!
	$field_args = array(
		'type'      => $type,
		'id'        => $id,
		'desc'      => $desc,
	...
	add_settings_field( $id, $title, 'maja_form_field_fn', __FILE__, $section, $field_args );
 ...
 */
function maja_register_settings(){
	
	// get the settings sections array
	$settings_output 	= maja_get_settings();
	$maja_option_name = $settings_output['maja_option_name'];
	
	//setting
	register_setting($maja_option_name, $maja_option_name, 'maja_validate_options' );
	
	//sections
	if(!empty($settings_output['maja_page_sections'])){
		// call the "add_settings_section" for each!
		foreach ( $settings_output['maja_page_sections'] as $id => $title ) {
			add_settings_section( $id, $title, 'maja_section_fn', __FILE__);
		}
	}
		
	//fields
	if(!empty($settings_output['maja_page_fields'])){
		// call the "add_settings_field" for each!
		foreach ($settings_output['maja_page_fields'] as $option) {
			maja_create_settings_field($option);
		}
	}
}
 ...
 */
function maja_settings_scripts(){
	wp_enqueue_style('maja_theme_settings_css', get_template_directory_uri() . '/lib/css/maja_theme_settings.css');
	wp_enqueue_script( 'maja_theme_settings_js', get_template_directory_uri() . '/lib/js/maja_theme_settings.js', array('jquery'));
}

/**
 * The Admin menu page
 */
function maja_add_menu(){
return;
	$settings_output = maja_get_settings();
	
	// Display Settings Page link under the "Appearance" Admin Menu
	// add_theme_page( $page_title, $menu_title, $capability, $menu_slug, $function);
	$maja_settings_page = add_theme_page(__('Maja Options', 'maja'), __('Theme Options','maja'), 'manage_options', MAJA_PAGE_BASENAME, 'maja_settings_page_fn');

		// css & js
		add_action( 'load-'. $maja_settings_page, 'maja_settings_scripts' );	
}
 ...
 * @return echoes output
 */
function maja_form_field_fn($args = array()) {
	...
	$maja_option_name = $settings_output['maja_option_name'];
	$options 			= get_option($maja_option_name);
	...
	switch ( $type ) {
		case 'text':
			...
			echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $maja_option_name . "[$id]' value='$options[$id]' />";
		...
		case "multi-text":
			foreach($choices as $item) {
				...
				echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $maja_option_name . "[$id|$item[1]]' value='$value' /><br/>";
			}
		...
		case 'textarea':
			...
			echo "<textarea style='width:300px; height:200px' class='textarea$field_class' type='text' id='$id' name='" . $maja_option_name . "[$id]' rows='5' cols='30'>$options[$id]</textarea>";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : ""; 		
		break;
		
		case 'select':
			echo "<select id='$id' class='select$field_class' name='" . $maja_option_name . "[$id]'>";
				foreach($choices as $item) {
					$value 	= esc_attr($item, 'maja');
					$item 	= esc_html($item, 'maja');
		...
		case 'checkbox':
			echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $maja_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
		...
		case "multi-checkbox":
			foreach($choices as $item) {
				...
				echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $maja_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";
			}
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";
		break;
	}
 ...
 * @return echoes output
 */
function maja_settings_page_fn() {
	// get the settings sections array
	$settings_output = maja_get_settings();
?>
	<div class="wrap">
		<div class="icon32" id="icon-options-general"></div>
		<h2><?php echo $settings_output['maja_page_title']; ?></h2>
		
		<form action="options.php" method="post">
			<?php 
			// http://codex.wordpress.org/Function_Reference/settings_fields
			settings_fields($settings_output['maja_option_name']); 
			// http://codex.wordpress.org/Function_Reference/do_settings_sections
			do_settings_sections(__FILE__); 
			?>
			<p class="submit">
				<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','maja'); ?>" />
			</p>
 ...
 * @return array
 */
function maja_validate_options($input) {
	...
	// collect only the values we expect and fill the new $valid_input array i.e. whitelist our option IDs
		...
		$options = $settings_output['maja_page_fields'];
					...
					switch ( $option['class']) {
						...
						case 'numeric':
							...
							if(is_numeric($input[$option['id']]) == FALSE) {
								add_settings_error(
									...
									MAJA_SHORTNAME . '_txt_numeric_error', // error ID
									__('Expecting a Numeric value! Please fix.','maja'), // error message
						...
						case 'multinumeric':
							...
							if($input[$option['id']] !=''){
								...
								$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','maja');
							}else{
							...
							if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
								add_settings_error(
									$option['id'], // setting title
									MAJA_SHORTNAME . '_txt_multinumeric_error', // error ID
									__('Expecting comma separated numeric values! Please fix.','maja'), // error message
									'error' // type of message
								);
							}
						break;
						...
						case 'email':
							...
							$input[$option['id']] 		= trim($input[$option['id']]); // trim whitespace
							if($input[$option['id']] != ''){
								$valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','maja');
							}elseif($input[$option['id']] == ''){
								$valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','maja');
							}
							...
							if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {
								add_settings_error(
									$option['id'], // setting title
									MAJA_SHORTNAME . '_txt_email_error', // error ID
									__('Please enter a valid email address.','maja'), // error message
									'error' // type of message
								);
							}
						break;
 ...
 */
function maja_show_msg($message, $msgclass = 'info') {
 ...
 *
 * @return calls maja_show_msg()
 */
function maja_admin_msgs() {
	
	// check for our settings page - need this in conditional further down
	$maja_settings_pg = strpos( 'appearance_page_' . MAJA_PAGE_BASENAME . ' ', MAJA_PAGE_BASENAME);
	...
	if(current_user_can ('manage_options') && $maja_settings_pg !== FALSE && !empty($set_errors)){

		// have our settings succesfully been updated? 
		if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){
			maja_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');
		...
		// have errors been found?
			...
			foreach($set_errors as $set_error){
				...
				maja_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');
			}
		}
	}
}
...
// admin messages hook!
add_action('admin_notices', 'maja_admin_msgs');
?>