<?php
/**
 * The template for displaying portfolio items (3 columns).
 * It can be loaded by placing [portfolio-3col] shortcode 
 * inside your page.
 */
?>
...
<ul class="filterable-grid portfolio_group col3_portfolio">
    ...
    <?php $wpbp = new WP_Query(array( 'post_type' => 'portfolio', 'posts_per_page' =>'-1') ); ?>
		...
		<?php if ($wpbp->have_posts()) : while ($wpbp->have_posts()) : $wpbp->the_post(); // The Loop ?>
            ...
            <li class="one_third" data-id="id-<?php echo $count; ?>" data-type="<?php if ($terms != '') foreach ($terms as $term) { echo strtolower(preg_replace('/\s+/', '-', $term->name)). ' '; } ?>">
                
                <?php if ( (function_exists('has_post_thumbnail')) && (has_post_thumbnail()) ) : ?>
                    
                    <?php if($item_url != '') { // if 'video url' box is empty display an image ?>  
                    
                        <a class="portfolio_thumbnail" rel="prettyPhoto[gallery]" href="<?php echo $item_url; ?>">
							<?php the_post_thumbnail('portfolio-thumbnail-3col'); ?>
                        </a> 
                        
                   <?php } else { // if there is an 'video url' display the video ?>
                   
                        <a class="portfolio_thumbnail" rel="prettyPhoto[gallery]" href="<?php echo $large_image ?>">
							<?php the_post_thumbnail('portfolio-thumbnail-3col'); ?>
                        </a>	
