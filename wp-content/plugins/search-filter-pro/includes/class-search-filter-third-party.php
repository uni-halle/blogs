...
class Search_Filter_Third_Party
{
	...
	public function sf_woocommerce_add_stock_status($ids_array, $query_args, $sfid) {
		...
		* check for hte woocommerce setting "show out of stock products", and only enable this on that condition
		...
		//$instock_ids = array(10843);
		//$ids_array = array_intersect($ids_array, $instock_ids);
		//var_dump($ids_array);exit;
		return $ids_array;
	}
	...
	public function sf_woo_post_cache_add_variation_product_data( $post ) {
		...
		foreach($product_variation_ids as $variation_id) {
			...
			//remove existing records for this variation
	...
	public function sf_woocommerce_cache_data($cache_data)
	{
		...
		if ((in_array("product", $cache_data['post_types'])) && (in_array("product_variation", $cache_data['post_types']))) {
			...
			//we also want to record `_stock_status` regardless if it has been set as a field - we need this because calc get complicated when checking if stock is managed at variation or product level
			//var_dump($cache_data);
