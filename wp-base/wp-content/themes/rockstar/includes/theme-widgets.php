...
?>
	...
	<div id="rockstartv" class="widget">

		<h3><?php _e('Latest Video',woothemes); ?></h3>
			...
			<?php
				...
				foreach($videos as $post) {
					echo woo_get_embed('embed','290','163');
				}
...
<div id="flickr" class="widget">
	<h3><?php _e('Flickr Photos',woothemes); ?></h3>
	<div class="wrap">
		<div class="fix"></div>
...
function widget_Twidget_init() {
	...
	function widget_Twidget($args) {
		...
		if ( $title <> "" ) { echo '<h3>' . $title . '</h3>'; } else { echo '<h3>' . __('Twitter Updates',woothemes) . '</h3>'; }
...
?>	
	...
	<div id="rss">
		...
		<a href="<?php if ( get_option('woo_feedburner_url') <> "" ) { echo get_option('woo_feedburner_url'); } else { echo get_bloginfo_rss('rss2_url'); } ?>" title="<?php _e('subscribe to rss',woothemes); ?>"><?php _e('subscribe to rss',woothemes); ?></a>
	
	</div><!--rss-->

<?php 	
}
register_sidebar_widget('Woo - RSS', 'rssWidget');
